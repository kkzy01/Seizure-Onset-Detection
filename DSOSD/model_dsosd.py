...
from utils_dsosd import num_wins, MovingWinClips
import warnings
...
class DSOSDbase:
    ...
    def get_onset_and_spread(self,sz_prob,threshold=None,
        ...
        sz_clf_filtered = sz_clf.apply(lambda col: sc.ndimage.median_filter(col, size=filter_w_idx, mode='nearest', origin=0), axis=0)

        seized_idxs = np.any(sz_clf_filtered,axis=0)
        ...
        if ret_smooth_mat:
            ...
            return sz_idxs_df,sz_clf_ff, sz_spread_idxs
        else:
            return sz_idxs_df
    ...
    def get_gaussianx_threshold(self,sz_prob,noise_floor='automedian',verbose=False,seed=100):
        ...
        if noise_floor == 'mean':
            ...
            if np.sum(all_gbounds > 1.194797045747334) == 0:
                threshold = 1.479305740987984
            else:
                threshold = np.nanmean(all_gbounds[all_gbounds > 1.194797045747334])
        elif noise_floor == 'automedian':
            if np.sum(all_gbounds > 1.1725) == 0:
                threshold = 1.479305740987984
            else:
                threshold = np.nanmedian(all_gbounds[all_gbounds > 1.194797045747334])
